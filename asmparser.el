(defun instruction-mov (operands)
  ...
  (setf (nth (car operands) memory) (nth (nth 1 operands) memory)))

(defun instruction-movi (operands)
  "Move immediate value to memory."
  (setf (nth (car operands) memory) (nth 1 operands)))

(defun instruction-add (operands)
  "Adds memory to memory, stores in memory."
  (setf (nth (car operands) memory) (+ (nth (nth 1 operands) memory) (nth (nth 2 operands) memory))))

(defun instruction-addi (operands)
  "Adds memory to immediate value, stores in memory."
  (setf (nth (car operands) memory) (+ (nth (nth 1 operands) memory) (nth 2 operands))))
...
(defun instruction-beqz (operands)
  ...
  (when (= (nth (car operands) memory) 0)
    (setf instruction-pointer (1- (nth 1 operands)))))
...
(defun instruction-display (operands)
  ...
  (message (nth 0 operands)))

(defun tokenize (operands)
  "Tokenizes operands."
  (let ((tokens '()))
    (dolist (token operands)
      (cond ((string-prefix-p "$" token) (setf tokens (append tokens (list (list 'int (string-to-number (mapconcat 'identity (cdr (split-string token "" t)) "")))))))
	    ((string-prefix-p "!" token) (setf tokens (append tokens (list (list 'string (mapconcat 'identity (cdr (split-string token "" t)) ""))))))
	    (t (setf tokens (append tokens (list (list 'cell (string-to-number token))))))))
    tokens))
...
(defun parse-line (line)
  (let* ((parts (split-string line " " t))
    ...
    (cond ((string= operation "MOV")
	   (cond ((matches? operands (list 'cell 'cell)) (setf instruction-list (append instruction-list (list (list 'mov (get-values operands))))))
		 ((matches? operands (list 'cell 'int)) (setf instruction-list (append instruction-list (list (list 'movi (get-values operands))))))))
	  ((string= operation "ADD")
	   (cond ((matches? operands (list 'cell 'cell 'cell)) (setf instruction-list (append instruction-list (list (list 'add (get-values operands))))))
		 ((matches? operands (list 'cell 'cell 'int)) (setf instruction-list (append instruction-list (list (list 'addi (get-values operands))))))))
	  ((string= operation "MUL")
	   (cond ((matches? operands (list 'cell 'cell 'cell)) (setf instruction-list (append instruction-list (list (list 'mul (get-values operands))))))
		 ((matches? operands (list 'cell 'cell 'int)) (setf instruction-list (append instruction-list (list (list 'muli (get-values operands))))))))
	  ((string= operation "DIV")
	   (cond ((matches? operands (list 'cell 'cell 'cell)) (setf instruction-list (append instruction-list (list (list 'div (get-values operands))))))
		 ((matches? operands (list 'cell 'cell 'int)) (setf instruction-list (append instruction-list (list (list 'divi (get-values operands))))))))
	  ((string= operation "MOD")
	   (cond ((matches? operands (list 'cell 'cell 'cell)) (setf instruction-list (append instruction-list (list (list 'mod (get-values operands))))))
		 ((matches? operands (list 'cell 'cell 'int)) (setf instruction-list (append instruction-list (list (list 'modi (get-values operands))))))))
	  ((string= operation "BEQZ")
	   (cond ((matches? operands (list 'cell 'int)) (setf instruction-list (append instruction-list (list (list 'beqz (get-values operands))))))))
	  ((string= operation "BNEQZ")
	   (cond ((matches? operands (list 'cell 'int)) (setf instruction-list (append instruction-list (list (list 'bneqz (get-values operands))))))))
	  ((string= operation "DISPLAY")
	   (cond ((matches? operands (list 'string)) (setf instruction-list (append instruction-list (list (list 'display (get-values operands)))))))))))
...
(let ((memory (make-list 16 0))
      (instruction-list '()))
  (parse-line "DISPLAY !PRRPX")
  (parse-line "DISPLAY !IS")
  (parse-line "DISPLAY !A")
  (parse-line "DISPLAY !FAGGOT")
  (run))
